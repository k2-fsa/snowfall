...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default='{XS}',
        help='Subset of the GigaSpeech dataset.')
    return parser


def main():
    args = get_parser().parse_args()
    dataset_parts = (args.subset, '{TEST}', '{DEV}')
    print("Parts we will prepare: ", dataset_parts)

    corpus_dir = locate_corpus(
        Path('/export/corpora5/GigaSpeech'),
        Path('/home/storage07/zhangjunbo/data/giga'),
    )
    musan_dir = locate_corpus(
        ...
        Path('/home/storage07/zhangjunbo/data/musan'),
    )
    ...
    with get_executor() as ex:  # Initialize the executor only once.
        for partition, manifests in gigaspeech_manifests.items():
            ...
            cut_set.to_jsonl(output_dir / f'cuts_{partition}.jsonl.gz')
        # Now onto Musan
