...
def get_objf(
    ...
    tot_scores = target_graph.get_tot_scores(log_semiring=True, use_double_scores=True)
...
def get_validation_objf(
    ...
    total_all_frames = 0.0  # all frames including those seqs that failed.

    model.eval()

    for batch_idx, batch in enumerate(dataloader):
        objf, frames, all_frames = get_objf(batch, model, device, graph_compiler, False)
        total_objf += objf
        total_frames += frames
...
def main():
    ...
    cuts_dev = CutSet.from_file(feature_dir / "gigaspeech_cuts_DEV.jsonl.gz").subset(first=1000)
