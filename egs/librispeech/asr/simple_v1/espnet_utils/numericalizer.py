...
class NumericalizerMixin(ABC):
    ...
    def _assign_special_symbols(self):
        ...
        self.sos_idx = self.token2idx[
            '<sos/eos>'] if '<sos/eos>' in self.token2idx else self.token2idx[
                '<sos>']
        self.eos_idx = self.token2idx[
            '<sos/eos>'] if '<sos/eos>' in self.token2idx else self.token2idx[
                '<eos>']
...
class SpmNumericalizer(NumericalizerMixin):
    ...
    def ids2tokens(self, integers: Union[np.ndarray,
                                         Iterable[int]]) -> List[str]:
